...
const Sidebar: React.FC<SidebarProps> = ({ user, currentView, setView, onLogout }) => {
  ...
  const adminItems = [
    { id: 'analytics', label: 'Análisis Pro', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
    )},
    { id: 'timeline', label: 'Historial Ventas', icon: (
    ...
    { id: 'discounts', label: 'Ofertas/Precios', icon: (
  ...
  const handleSignOut = async () => {
    ...
    setExiting(true);
    try {
      console.log("Intentando registrar LOGOUT para:", user.username);
      const { error } = await supabase.from('audit_logs').insert({
        usuario_id: user.id,
        accion: 'LOGOUT',
        modulo: 'SISTEMA',
        detalles: `Salida: ${new Date().toLocaleTimeString()}`
      });
      
      if (error) {
        console.error("❌ Error al registrar salida en BD:", error);
      } else {
        console.log("✅ Logout registrado en BD");
      }
    } catch (e) {
      console.error("❌ Excepción al registrar salida:", e);
    } finally {
      onLogout();
    }
  };
        ...
        {isSuperUser && (
           <div className="mt-8">
             <button
               ...
               className={`w-full flex items-center gap-4 px-5 py-3.5 rounded-2xl transition-all font-black text-[13px] uppercase tracking-tight border border-violet-500/30 ${
        ...
        <div className="bg-slate-900/50 rounded-3xl p-5 border border-slate-800/50 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              ...
              <p className={`text-[9px] uppercase tracking-widest font-bold ${isSuperUser ? 'text-violet-400' : 'text-slate-500'}`}>{isSuperUser ? 'Super Usuario' : isAdmin ? 'Administrador' : 'Operario'}</p>
            </div>
          </div>
        </div>
        <button
