...
const Sidebar: React.FC<SidebarProps> = ({ user, currentView, setView, onLogout }) => {
  ...
  if (user && user.role_id === 1) {
    menuItems.push({ id: 'timeline', label: 'Historial', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    )});
    menuItems.push({ id: 'analytics', label: 'Rentabilidad', icon: (
    ...
    menuItems.push({ id: 'users', label: 'Usuarios', icon: (
  ...
  return (
    <aside className="w-72 bg-slate-950 text-slate-300 flex flex-col hidden lg:flex border-r border-slate-900">
      ...
      <div className="p-6">
        ...
        <button
          onClick={onLogout}
          className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-slate-500 hover:bg-rose-500/10 hover:text-rose-400 transition-all font-bold text-sm"
        >
