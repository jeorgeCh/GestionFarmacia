...
const App: React.FC = () => {
      ...
      if (savedUser) {
        ...
        if (parsed && parsed.id) {
          const { data, error } = await supabase
            .from('usuarios')
            .select('id, role_id, activo, username')
            .eq('id', parsed.id)
            .maybeSingle();
  ...
  // Admin incluye rol 1 (admin) y rol 3 (super usuario)
  const isAdmin = Number(user.role_id) === 1 || isSuperUser;

  const renderView = () => {
    switch (view) {
      case 'dashboard': return <Dashboard user={user} setView={setView} />;
      case 'inventory': return <Inventory user={user} setView={setView} />;
      ...
      case 'superadmin': return isSuperUser ? <SuperAdmin /> : <div className="p-12 text-center text-rose-500 font-black uppercase tracking-widest text-xs">Acceso Restringido - Nivel 3 Requerido</div>;
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
        ...
        <header className="mb-6 flex justify-between items-center bg-white p-5 lg:p-6 rounded-[2rem] shadow-sm border border-slate-100">
          <div>
            <h1 className="text-xl lg:text-2xl font-black text-slate-900 tracking-tight uppercase leading-none">
              {view === 'pos' ? 'Ventas' : view === 'income' ? 'Ingresos' : view === 'inventory' ? 'Inventario' : view === 'discounts' ? 'Descuentos' : view === 'providers' ? 'Proveedores' : view === 'timeline' ? 'Historial' : view === 'analytics' ? 'Análisis' : view === 'superadmin' ? 'Super Usuario' : 'Panel'}
            </h1>
          ...
          <div className="flex items-center gap-3">
             <div className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${isSuperUser ? 'bg-violet-50 text-violet-600 border border-violet-100' : isAdmin ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600'}`}>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 py-3 flex items-center z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] backdrop-blur-md overflow-x-auto custom-scrollbar px-4 gap-6 no-scrollbar">
        <button onClick={() => setView('dashboard')} className={`flex flex-col items-center gap-1 min-w-[50px] transition-all ${view === 'dashboard' ? 'text-slate-900 font-black scale-110' : 'text-slate-300'}`}>
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
          <span className="text-[7px] uppercase tracking-tighter">Panel</span>
        </button>
        <button onClick={() => setView('pos')} className={`flex flex-col items-center gap-1 min-w-[50px] transition-all ${view === 'pos' ? 'text-emerald-600 font-black scale-110' : 'text-slate-300'}`}>
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
          <span className="text-[7px] uppercase tracking-tighter">Venta</span>
        </button>
        <button onClick={() => setView('inventory')} className={`flex flex-col items-center gap-1 min-w-[50px] transition-all ${view === 'inventory' ? 'text-slate-900 font-black scale-110' : 'text-slate-300'}`}>
        ...
        {isAdmin && (
           <button onClick={() => setView('analytics')} className={`flex flex-col items-center gap-1 min-w-[50px] transition-all ${view === 'analytics' ? 'text-indigo-600 font-black scale-110' : 'text-slate-300'}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
            <span className="text-[7px] uppercase tracking-tighter">Análisis</span>
          </button>
        )}
      </nav>
      <style>{`
